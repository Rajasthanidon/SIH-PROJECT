...
import { fetchAdminJobs } from '../../services/adminApi';
...
export default function AdminJobsPage() {
  ...
  const activeCount = jobs.filter(j => j.status === 'ACTIVE' || j.status === 'PUBLISHED').length;
  const draftCount = jobs.filter(j => j.status === 'DRAFT').length;
  const closedCount = jobs.filter(j => j.status === 'CLOSED' || j.status === 'EXPIRED').length;
    ...
    <div className="page-content relative">
      ...
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <p className="text-slate-600 mt-2">Manage and monitor all full-time job opportunities.</p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-600">
            ...
            <tbody className="divide-y divide-slate-200">
              {loading ? (
                <tr>
                  <td colSpan="6" className="px-6 py-10 text-center">Loading jobs...</td>
                </tr>
              ) : filteredJobs.length === 0 ? (
                <tr>
                  <td colSpan="6" className="px-6 py-10 text-center">No jobs found.</td>
                </tr>
              ) : (
                filteredJobs.map(job => (
                  <tr key={job.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-slate-500">{job.employment_type || 'Full Time'}</div>
                    </td>
                    <td className="px-6 py-4 font-medium text-slate-700">{job.company_name}</td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <button 
                        onClick={() => setSelectedOpportunityId(job.id)}
